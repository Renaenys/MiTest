	...
	for (const bot of bots) {
		...
		try {
			...
			console.log(
				`🧮 Filled Layers: ${
					filledOrders.length
				}, Avg Price: ${averagePrice.toFixed(6)}`
			);
			console.log(
				`💰 USDT Balance: ${usdtBalance}, ${baseCurrency} Balance: ${baseBalance}`
			);
			...
			console.log(`🎯 Next TP Target: ${tpPrice.toFixed(6)} USDT`);
			...
			if (currentLayer === 0 && usdtBalance >= dcaOrderSize) {
				...
				console.log(
					`✅ New Session: Market Buy ${amount} ${baseCurrency} at ${currentPrice}`
				);
				continue;
			}
			...
			if (totalAmount > 0 && currentPrice >= tpPrice) {
				...
				await Order.updateMany(
					{ user: bot.user, symbol, status: 'open' },
					{
						$set: {
							status: 'closed',
							profit: sellOrder.cost - totalCost,
							closedAt: new Date(),
						},
					}
				);
				console.log(
					`🎯 TP HIT! Sold ${totalAmount} ${baseCurrency} at ${currentPrice} for profit.`
				);
			...
			if (currentLayer < bot.dcaMaxLayers) {
				...
				console.log(
					`🔻 Next DCA Layer Price: ${nextLayerPrice.toFixed(6)} USDT`
				);
				if (currentPrice <= nextLayerPrice && usdtBalance >= dcaOrderSize) {
					const amount = dcaOrderSize / nextLayerPrice;
					const limitOrder = await exchange.createLimitBuyOrder(
						symbol,
						amount,
						nextLayerPrice
					);
					...
					console.log(
						`✅ DCA Layer ${
							currentLayer + 1
						} Added: ${amount} ${baseCurrency} at ${nextLayerPrice}`
					);
				} else {
				...
				console.log(
					`⛔ Max DCA layers reached (${currentLayer}/${bot.dcaMaxLayers})`
				);
			}
		} catch (error) {
